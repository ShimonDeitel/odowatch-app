...
final class Store: ObservableObject {
    @Published private(set) var items: [Reading] = []
    ...
    init() {
        ...
        fileURL = support.appendingPathComponent("odowatch_items.json")
        load()
        if items.isEmpty {
            seed()
        }
    ...
    func add(title: String, detail: String = "") {
        let item = Reading(title: title, detail: detail)
    ...
    func toggleDone(_ item: Reading) {
        guard let idx = items.firstIndex(where: { $0.id == item.id }) else { return }
    ...
    private func seed() {
        items = [
            Reading(title: "First entry", detail: "Sample seeded record"),
            Reading(title: "Second entry", detail: "Another sample record")
        ]
        save()
    }
